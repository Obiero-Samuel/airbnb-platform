...
CREATE TRIGGER before_reservation_update BEFORE
...
CREATE TRIGGER before_property_insert BEFORE
...
CREATE PROCEDURE GetUserReservationStats() BEGIN
SELECT u.user_id,
    u.username,
    u.email,
...
GRANT SELECT,
    INSERT,
    UPDATE ON airbnb_system.properties TO 'host_user' @'localhost';
GRANT SELECT,
    UPDATE ON airbnb_system.reservations TO 'host_user' @'localhost';
...
SELECT p.property_id,
    ...
    SUM(
        CASE
            WHEN r.status = 'confirmed' THEN r.total_price
            ELSE 0
        END
    ) as total_revenue
FROM properties p
